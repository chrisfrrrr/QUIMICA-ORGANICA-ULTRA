...
st.set_page_config(
    page_title="IA Química Orgánica ULTRA",
    page_icon="💡",
    layout="wide"
)


def show_molecule(mol, caption="Molécula", width=380):
    ...
    if img is not None:
        ...
        st.warning(
            "No se pudo generar la imagen de la molécula, "
            "pero la estructura sí fue interpretada correctamente."
        )
...
st.caption("Desarrollado por: Christian David Pocol Franco | Ingeniero Electrónico")
...
with tabs[0]:
    ...
    Si el nombre no está en el diccionario local, la app puede consultar PubChem o NCI/Cactus.
    """)

    st.subheader("Qué puedes hacer")
    st.markdown("""
    - Dibujar estructuras desde nombre IUPAC o SMILES.
...
with tabs[1]:
    st.header("✍️ Dibujar estructura desde nombre IUPAC, nombre común o SMILES")

    ex = example_select("iupac")
    name = st.text_input("Nombre IUPAC / común / SMILES", ex["name"], key="iupac_name")
    use_pubchem = st.checkbox("Usar PubChem/NCI si no está localmente", value=True, key="iupac_pubchem")
    ...
    if st.button("Convertir y dibujar", key="btn_iupac"):
        ...
        if not smiles:
            ...
            if mol is None:
                st.error("Se obtuvo un SMILES, pero RDKit no pudo interpretarlo.")
            else:
                ...
                with c1:
                    show_molecule(mol, caption=name, width=380)
...
with tabs[2]:
    ...
    input_mode = st.selectbox(
        "Tipo de entrada",
        ["SMILES", "Nombre / IUPAC"],
        index=0 if ex["input_mode"] == "SMILES" else 1,
        key="an_mode"
    )

    molecule = st.text_input("Molécula", ex["molecule"], key="an_mol")
    use_pubchem = st.checkbox("Permitir PubChem/NCI", value=True, key="an_pubchem")

    if st.button("Analizar molécula", key="btn_analyze"):
        mol, msg, resolved_smiles = parse_molecule(
            molecule,
            input_mode,
        ...
        if mol is None:
            ...
            with c1:
                show_molecule(mol, caption="Estructura", width=350)
            ...
            with c2:
                ...
                st.dataframe(
                    pd.DataFrame(report.items(), columns=["Propiedad", "Valor"]),
...
with tabs[3]:
    ...
    use_pubchem = st.checkbox("Permitir PubChem/NCI", value=True, key="rxn_pubchem")
    ...
    if st.button("Clasificar reacción", key="btn_rxn"):
        ...
        if mol is None:
            ...
            with left:
                show_molecule(mol, caption="Sustrato", width=350)
                st.code(resolved_smiles or Chem.MolToSmiles(mol), language="text")
            ...
            if products:
                ...
                for i, p in enumerate(products):
                    ...
                    with cols[i % len(cols)]:
                        if pmol:
                            show_molecule(pmol, caption=p["label"], width=300)
...
with tabs[4]:
    ...
    use_pubchem = st.checkbox("Permitir PubChem/NCI", value=True, key="ar_pubchem")

    if st.button("Analizar aromaticidad", key="btn_ar"):
        mol, msg, _ = parse_molecule(
            ar_text,
            ar_mode,
            use_pubchem=use_pubchem
        )
        ...
        if mol is None:
            ...
            with c1:
                show_molecule(mol, caption="Molécula", width=350)
...
with tabs[5]:
    ...
    use_pubchem = st.checkbox("Permitir PubChem/NCI", value=True, key="st_pubchem")
...
with tabs[6]:
    ...
    if st.button("Comparar", key="btn_cmp"):
        ...
        if m1 is None:
            ...
            with c1:
                show_molecule(m1, caption="Compuesto A", width=300)

            with c2:
                show_molecule(m2, caption="Compuesto B", width=300)
